...
fo = open("Custom_Grid_Search_Out_Cross_Validation_GMM_Data_Only_Parse_Entrance.csv", 'w', newline='', encoding='utf-8')
fo.writelines(
    'n_components,max_iter,covariance_type,sensors_input,true positive rate c1,true positive rate c2,true negative rate c1,'
    'true negative rate c2,positive predictive value c1,positive predictive value c2,Negative predictive value c1,'
    'Negative predictive value c2,false positive rate c1,false positive rate c2,False negative rate c1,False negative '
    'rate c2,False discovery rate c1,False discovery rate c2,Overall accuracy c1,Overall accuracy c2,class_majority,time_window\n')
...
for x in timeintervals:
    ...
    # define dataset
    df = pd.read_csv(folder + file + x + "_Seconds_Count_Data_Only_Parse_Entrance.csv")
    df_3 = pd.read_csv(folder_3 + file_3 + x + "_Seconds_Count_Data_Only_Parse_Entrance.csv")
