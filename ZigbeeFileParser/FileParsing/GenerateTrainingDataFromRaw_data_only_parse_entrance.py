...
for z in timewindow:
    ...
    f.close()
    f = open(folder + "\\" + file, "r")
    foc = open(folder + "\\" + file + "_" + str(z) + "_Seconds_Count_Data_Only_Parse_Entrance.csv", "w+")
    ...
    for x in f:
        ...
        if not split[0] == "TimeStamp":
            if int(split[6].rstrip()) == 1:
                ...
                if timestamp > (occupied_windows_datetime - td(seconds=300)) and occupied != 1:
                    occupied = 1
                if timestamp > (occupied_windows_datetime + td(
                    seconds=300)) and occupied_windows_index < len(occupied_windows) and occupied == 1:
                    occupied = 0
