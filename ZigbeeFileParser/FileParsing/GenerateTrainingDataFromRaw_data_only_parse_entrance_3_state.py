...
for z in timewindow:
    ...
    for x in f:
        ...
        if not split[0] == "TimeStamp":
            if first <= 0:
                delta = datetime.strptime(split[0], '%Y-%m-%d %H:%M:%S') + td(seconds=z)
                first = 1
            if split[11] == '0x1c88':
    ...
    f.close()
    f = open(folder + "\\" + file, "r")
    foc = open(folder + "\\" + file + "_" + str(z) + "_Seconds_Count_Data_Only_Parse_Entrance_3_State.csv", "w+")
    ...
    for x in f:
        split = x.rstrip().split(',')
        if not split[0] == "TimeStamp":
            if int(split[6].rstrip()) == 1:
                timestamp = datetime.strptime(split[0], '%Y-%m-%d %H:%M:%S')
                if timestamp > (occupied_windows_datetime - td(seconds=60)) and occupied != 1 and done != 1:
                    if occupied == 2:
                        exit = 1
                    else:
                        exit = 0
                    occupied = 1
                if timestamp > (occupied_windows_datetime + td(
                    seconds=60)) and occupied_windows_index < len(occupied_windows) and occupied == 1:
                    if exit == 1:
                        occupied = 0
                        exit = 0
                    else:
