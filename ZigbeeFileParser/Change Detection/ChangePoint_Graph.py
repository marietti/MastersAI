...
def R_ULSIF(x_de, x_nu, x_re, x_ce, alpha_rulsif=0.5, fold=5):
    ...
    for i in range(0, len(sigma_list_rulsif)):
        ...
        for j in range(0, len(lambda_list_rulsif)):
            ...
            for k in range(1, fold + 1):
                ...
                temp = H_k + temp
                temp_out = np.nan_to_num(temp)
                h_k_out = np.nan_to_num(h_k)
                theta = linalg.solve(temp_out, h_k_out)
                # theta = max(theta, 0)
    ...
    h = np.mean(k_nu, 1)
    temp_out = np.nan_to_num((H + eye(n_ce) * lambda_chosen))
    h = np.nan_to_num(h)
    theta = linalg.solve(temp_out, h)
...
sensor = "0xbfb8"
window_size = 600
timeintervals = ["5", "10", "30", "45", "60", "75", "90", "105", "120"]
timeinterval_n = [60, 30, 10, 6, 5, 4, 3, 2, 2]
timeinterval_k = [30, 15, 5, 3, 2, 2, 1, 1, 1]
index = 0


df = pd.read_csv("C:\\Users\\nickb\source\\repos\\MastersAI\\ZigbeeFileParser\\FileParsing\\Training\\cc2531_sniffer_2_3_2021_Trainingout_120_Seconds_Count.csv")
...
y = np.reshape(np.array(df[sensor].tolist()), (1, np.array(df[sensor].tolist()).size))
...
y_axis = y_shape[0]
alpha_list = [0, .1, .01, .001, .0001]

n = timeinterval_n[index]
k = timeinterval_k[index]
index += 1
subplot(2, 1, 1)
plt.plot(y_trans)
plt.plot([element * max(y_trans)/4 for element in occupied], label="Occupied")
plt.legend()
index = 1
for alpha in alpha_list:
    [score1, _, _] = change_detection(y, n, k, alpha, 5)
    [score2, _, _] = change_detection(np.array(Reverse(y)), n, k, alpha, 9)

    subplot(3, 2, index)
    final_score = score1 + score2
    zeros_score = zeros(n+k)
    plot_array = np.concatenate((zeros_score, final_score))
    plot_array_1 = np.concatenate((zeros_score, score1))
    plot_array_2 = np.concatenate((zeros_score, score2))
    # axis([0, y_axis, 0, 100])
    title('Change-Point Score')
    plt.plot(plot_array, label="Symmetric" + str(alpha))
    ...
    plt.plot([element * max(plot_array)/2 for element in occupied], label="Occupied")
    plt.plot([element * max(plot_array)/3 for element in predicted], label="Predicated")
    index += 1
    plt.legend()
show()
